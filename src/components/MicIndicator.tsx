interface MicIndicatorProps {
  state: 'idle' | 'listening' | 'disabled';
  onClick?: () => void;
}

export function MicIndicator({ state, onClick }: MicIndicatorProps) {
  const colors = {
    idle: 'var(--bg-secondary)',
    listening: 'var(--danger)',
    disabled: 'var(--text-secondary)',
  };

  return (
    <button
      onClick={onClick}
      disabled={state === 'disabled'}
      className={`flex items-center justify-center rounded-full border-none cursor-pointer transition-all ${
        state === 'listening' ? 'animate-pulse' : ''
      }`}
      style={{
        width: 'var(--tap-target-child)',
        height: 'var(--tap-target-child)',
        backgroundColor: colors[state],
        opacity: state === 'disabled' ? 0.5 : 1,
      }}
      aria-label={state === 'listening' ? 'Recording...' : 'Tap to record'}
    >
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={state === 'listening' ? 'white' : 'var(--text-primary)'} strokeWidth="2">
        <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" />
        <path d="M19 10v2a7 7 0 0 1-14 0v-2" />
        <line x1="12" y1="19" x2="12" y2="23" />
        <line x1="8" y1="23" x2="16" y2="23" />
      </svg>
    </button>
  );
}
